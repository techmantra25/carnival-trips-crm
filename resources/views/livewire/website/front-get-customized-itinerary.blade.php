            ...
            <div class="container">
                <div class="row">
                    <div class="col-xl-8 col-lg-10 col-12">
                        ...
                        <h1>Hello {{$leadData->customer_name}}</h1>
                        <h2>Here Is Your Customized Itinerary</h2>
                        <div class="duration">
                            <span class="text">Refreshing</span>
                            <span class="time"><img src="{{asset('front_assets/icons/clock-white.png')}}"
                                    alt="">{{$leadData->travel_duration_text}}</span>
                        </div>
                        <div class="location">
...
@if($template && $template->detail)
    ...
    @php
        ...
        $trip_highlights = $sent_lead_itinerary->details()
    ...
    <div class="content">
        <div class="container">
            <div class="row">
                    ...
                    <div class="tab-content feature-tabs-content">
                        <div class="tab-pane active" id="itinerary" role="tabpanel" aria-labelledby="itinerary-tab">
                            <div class="itinerary-content">
                                ...
                                <h2 class="text-center">Your Customized Itinerary</h2>
                                <h2 class="text-center">Day by Day Brief</h2>

                            <div class="content-wrap-box">
                                <div class="accordion itinerary-content-accordion" id="itineraryAccordion1">
                                    @foreach (explode(',',$sent_lead_itinerary->stay_by_journey) as $division_key=> $division_item)
                                        @php
                                            $division = App\Models\City::find($division_item);
                                            $headingId = 'heading_'.$division_key;
                                            $collapseId = 'collapse_'.$division_key;
                                            $get_selected_hotel = App\Models\SendedLeadItineraryDetail::where('sended_lead_itinerary_id', $sent_lead_itinerary->id)
                                            ->where('day', $division_key+1)
                                            ->where('field', 'day_room')
                                            ->whereNotNull('room_id')
                                            ->first();
                                        
                                        $selected_hotel = [
                                            'hotel_id'    => $get_selected_hotel->hotel->id ?? null,
                                            'hotel_image' => !empty($get_selected_hotel->hotel->image)
                                                                ? $get_selected_hotel->hotel->image
                                                                : 'build/assets/images/logo/demo.webp',
                                            'hotel_name'  => $get_selected_hotel->hotel->name ?? null,
                                            'room_name'   => $get_selected_hotel->value ?? null,
                                            'type'        => 'selected',
                                        ];
                                        $extra_hotels_and_rooms = $get_selected_hotel->extra_hotels_and_rooms ?? [];

                                        // If JSON string → decode
                                        if (is_string($extra_hotels_and_rooms)) {
                                            $extra_hotels_and_rooms = json_decode($extra_hotels_and_rooms, true) ?? [];
                                        }

                                        // Ensure array
                                        if (!is_array($extra_hotels_and_rooms)) {
                                            $extra_hotels_and_rooms = [];
                                        }
                                       $division_hotels = [];

                                        // First extra (if exists)
                                        if (isset($extra_hotels_and_rooms[0])) {
                                            $division_hotels[] = $extra_hotels_and_rooms[0];
                                        }

                                        // Selected (always)
                                        $division_hotels[] = $selected_hotel;

                                        // Second extra (if exists)
                                        if (isset($extra_hotels_and_rooms[1])) {
                                            $division_hotels[] = $extra_hotels_and_rooms[1];
                                        }
                                                ...
                                                <button class="accordion-button collapsed" type="button"
                                                    ...
                                                    <div class="accordion-head">
                                                        ...
                                                        <div class="accordion-head-info-box">
                                                            @if($index < $sent_lead_itinerary->total_days)
                                                                <div class="inner-info">
                                                                    <img src="{{asset('front_assets/icons/bed-white.png')}}" alt="">
                                                                    <span>{{count($division_hotels)}}</span>
                                                                </div>
                                                            @endif
                                                        </div>
                                            ...
                                            <div id="{{$collapseId }}" class="accordion-collapse collapse"
                                                ...
                                                <div class="accordion-body">
                                                    ...
                                                    @if($index < $sent_lead_itinerary->total_days)
                                                        @if (isset($division_hotels) && count($division_hotels) > 0)
                                                            ...
                                                            <ul class="stay-category-tabs">
                                                                <li>
                                                                    <a href="javascript:void(0)"
                                                                        class="active">{{ $sent_lead_itinerary->category->name ?? '' }}
                                                            ...
                                                            <div class="itinerary-hospitalities">
                                                                <div class="info-header">
                                                                    ...
                                                                    <span>Total {{count($division_hotels)}}
                                                                        Selected</span>
                                                                </div>
                                                                <ul class="hospitalities-list">

                                                                    @foreach ($division_hotels as $hotels_key =>$division_hotels_item)
                                                                        <li>
                                                                            <div class="hospitality-card selected">
                                                                                <div class="img-wrap">
                                                                                    <img src="{{asset($division_hotels_item['hotel_image'])}}" alt="Hospitality">
                                                                                </div>
                                                                                <div class="info-wrap">
                                                                                    <div class="title">{{$division_hotels_item['hotel_name'] ?? ''}}</div>
                                                                                    <div class="bottom-row">
                                                                                        <h4> {{$division_hotels_item['room_name'] ?? ''}}</h4>
                                                                                        @if($division_hotels_item['type']=="selected")
                                                    ...
                                                    @php
                                                        ...
                                                        $day_routes = App\Models\SendedLeadItineraryDetail::where('sended_lead_itinerary_id',$sent_lead_itinerary->id)->where('header','day_'.$index)->where('field', 'day_route')->orderBy('id','ASC')->whereNotNull('route_service_summary_id')->get();
                                                    @endphp
                                                    @foreach($day_routes as $dayKey => $dayData)

                                                        @php
                                                            $route_service_summary_id = $dayData->route_service_summary_id;
                                                            $route_service_wise = App\Models\SendedLeadItineraryDetail::where('sended_lead_itinerary_id',$sent_lead_itinerary->id)
                ...
                <div class="tab-pane mb-2" id="inclusion" role="tabpanel" aria-labelledby="inclusion-tab">
                    <div class="inclusion-content">
                        @php
                        $inclusions = $sent_lead_itinerary->details()
                ...
                <div class="tab-pane mb-2" id="exclusion" role="tabpanel" aria-labelledby="exclusion-tab">
                    <div class="exclusion-content">
                        @php
                        $exclusions = $sent_lead_itinerary->details()
                        ->where('header', 'inclusion_exclusions')
