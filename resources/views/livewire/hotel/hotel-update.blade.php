                        ...
                        <div class="container mx-auto px-4 new-activity">
                            ...
                            <div class="mt-1 itinerary-build">
                                ...
                                <div class="grid grid-cols-3 gap-2 mt-3 image-preview-container">
                                    @foreach ($existingImagePreviews as $preview_old_image)
                                        <div class="relative image-preview peer-checked:border-blue-500 relative !overflow-visible"
                                            wire:key="existing-image-{{ $preview_old_image->id }}">
                                            
                                            <img src="{{ asset($preview_old_image->image_path) }}" alt="Preview" class="image-thumbnail cursor-pointer">
                                            
                                            <button type="button"
                                                    wire:click="removeOldImage({{ $preview_old_image->id }})"
                                                    class="delete-icon">
                                                ✖
                                            </button>
                                        </div>
                ...
                <div class="grid grid-cols-12 gap-3">
                    <div class="col-span-12">
                        ...
                        @foreach ($rooms as $index => $room)
                            <div class="sm:flex flex-wrap rounded-sm mt-2" wire:key="existing-image-{{ $room['id'] }}">
                                ...
                                <div class="w-full sm:w-auto w-12">
                                    ...
                                    <div class="select-group">
                                        ...
                                        <div class="list-group">
                                            @foreach ($hotel_ammenities as $ammenity_index=>$ammenity)
                                                <label>
                                                    <input type="checkbox"
                                ...
                                @if ($index > 0)
                                    @if($room['id'])
                                        <div class="w-full sm:w-auto w-8">
                                            <button type="button" wire:click="removeExistingRoom({{ $room['id'] }})"
                        ...
                        <div class="flex justify-end">
                            <button type="submit" class="change-text-button ti-btn ti-btn-primary-full !py-1 !px-2 ti-btn-wave me-[0.375rem]"> Update Hotel </button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    window.addEventListener('showConfirm', function (event) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                @this.call('deleteRoomItem', itemId); // Calls Livewire method directly
